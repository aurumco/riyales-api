    ...
    def compute_aggregates(self, endpoint: str, intervals: dict) -> None:
        ...
        for rec in raw_data:
            ...
            try:
                ...
                continue
        # Current time for window calculations (UTC) and for timestamp formatting (Tehran)
        now_utc = datetime.now(pytz.utc)
        now_local = now_utc.astimezone(LOCAL_TZ)
        # Compact timestamp without microseconds or offset
        now_str = now_local.strftime('%Y-%m-%dT%H:%M:%S')
        # Determine retention and max interval
        retention = timedelta(days=3)
        # Compute and persist per-interval, per-symbol time series
        for name, delta in intervals.items():
            # Window based on UTC timestamps
        ...
        # Purge raw_data older than retention period (3 days)
        cutoff_old = now_utc - retention
